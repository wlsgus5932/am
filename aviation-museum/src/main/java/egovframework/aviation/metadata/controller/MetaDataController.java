...
public class MetaDataController {
	...
	public String getMetaDataList(Model model) throws Exception {
		try {
			...
			return "metadata/search/metaDataList";
		}  catch (Exception e) {
...
//		List<Map<String, String>> list = service.getMovement();
		List<MovementVO> list = service.getMovement(item_idx);
		//List<MovementInVO> list2 = service.getMovementOut(movement_id);
		
		/*
		 * list.get(0).setStorage_out_nm(list2.get(0).getStorage_nm());
		 * list.get(0).setStorage_code_out_idx(list2.get(0).getStorage_code_idx());
		 */
